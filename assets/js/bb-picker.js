...
(function () {
  ...
  // Normaliza etiquetas: primera letra de cada palabra en mayúscula
  ...
  function mountSuggest(root, input, box, itemsFetcher, nextInput) {
    ...
    function pick(val) {
      // Siempre normalizamos lo que va al input (Chevrolet, Honda, etc.)
    ...
    var fire = debounce(function () {
      ...
      itemsFetcher(q, function (list) {
        ...
        // 🔥 Aquí normalizamos cómo se ve cada opción,
        // pero conservamos el valor original en data-value.
        box.innerHTML = list.map(function (x) {
          var raw = String(x);
          var label = bbNormalizeLabel(raw);
          // escapamos comillas para el atributo
  ...
  var vpTypes = ["car", "truck", "multipurpose%20passenger%20vehicle%20(MPV)", "suv"],
      vpAllMakes = null;

  function fetchAllMakes() {
    if (vpAllMakes) return Promise.resolve(vpAllMakes);
    var urls = vpTypes.map(function (t) {
      return "https://vpic.nhtsa.dot.gov/api/vehicles/GetMakesForVehicleType/" + t + "?format=json";
    });
    return Promise.all(urls.map(function (u) {
      ...
      .then(function (all) {
        var set = {};
        all.forEach(function (js) {
          (js.Results || []).forEach(function (r) {
            if (r && r.MakeName) set[r.MakeName.trim()] = 1;
          });
        });
        vpAllMakes = Object.keys(set).sort(function (a, b) {
  ...
  function fetchModels(make, year) {
    ...
    return fetch(url, { mode: "cors", credentials: "omit" })
      ...
      .then(function (js) {
        var arr = [];
        (js.Results || []).forEach(function (r) {
          var n = r.Model_Name || r.ModelName;
          if (n) arr.push(n.trim());
        });
  ...
  function initOne(root) {
    ...
    // ✅ Si no hay errores del servidor, dejamos los campos vacíos
    // (evita que aparezcan rellenos en la primera carga)
    var hasErrors = root.querySelector(".notice.notice-error");
    if (!hasErrors) {
      iYear.value = "";
      iMake.value = "";
      iModel.value = "";
    ...
    // Botones "Use this vehicle" en la lista de guardados
    var useBtns = root.querySelectorAll(".bb-use-vehicle");
    if (useBtns && useBtns.length) {
      useBtns.forEach(function (btn) {
        btn.addEventListener("click", function () {
          var y = btn.getAttribute("data-year") || "";
          var m = btn.getAttribute("data-make") || "";
          var mo = btn.getAttribute("data-model") || "";
          ...
          // Ocultar sugerencias abiertas
          var suggests = root.querySelectorAll(".bb-suggest");
          suggests.forEach(function (s) { s.style.display = "none"; });

          // Auto-enviar el formulario como si pulsara "See my price"
          var form = root.querySelector("#bb-form");
          if (form) {
